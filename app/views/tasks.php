        ...
        @if (count($tasks) > 0)
            @foreach ($tasks as $task)
                <div class="task" id="task-item-{{ $task->get('id') }}">
                    ...
                    <div class="task-description" id="task-item-description-{{ $task->get('id') }}"><pre>{!! $plant_link . (($task->get('description')) ? UtilsModule::translateURLs($task->get('description')) : __('app.none')) !!}</pre></div>
